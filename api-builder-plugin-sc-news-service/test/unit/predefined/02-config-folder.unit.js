...
test('### Ensure that generator.js file is present and not empty ###', t => {
  ...
  testState.scope = testState.connectorName.startsWith('@') ? testState.connectorName.substring(0, testState.connectorName.indexOf('/')) : undefined
  testState.shortName = testState.scope ? testState.connectorName.substring(testState.scope.length + 1) : testState.connectorName
...
test('### Ensure all required files in config folder exist ###', t => {
  t.ok(fs.existsSync(path.join(folderPath, `${testState.shortName}.default.js`)))
